# Adopted from https://github.com/hszhao/semseg/blob/master/util/transform.py
...
class ResizeAndCenterCrop(object):
    ...
    def __call__(self, img, label):
        ...
        label = self.apply_transforms(label)
        return img, label

    def apply_transforms(self, array):
        ...
        transformed_array = np.array(pil_img)
...
class FiveCropTransform:
    ...
    def __call__(self, img, label):
        img = Image.fromarray(img)# img and label are np array
        label = Image.fromarray(label)
        img_crops = torch.stack([self.normalize(self.to_tensor(crop)) for crop in self.five_crop(img)])
        label_crops = torch.stack([torch.tensor(np.array(crop)).long() for crop in self.five_crop(label)])
